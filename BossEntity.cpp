...
void BossEntity::updateAttacking()
{
    ...
    if(attackCounter == 10)
    {
        ...
        {
            getScene()->addNode(factory->makeEntity(ENTITY_BOSS_BULLET, getX(), getY()+i));
        }
    }
...
void BossEntity::updatePanic()
{
    ...
    {
        getScene()->addNode(factory->makeEntity(ENTITY_BOSS_BULLET, getX(), getY()+(rand()%getHeight())));
    }
...
void BossEntity::draw(Rectangle* view, Graphics* g)
{
    if(active)
    {
      ...
      int barWidth = (((getHealth() * 100)/ 500) * LIFEBAR_WIDTH) / 100;
