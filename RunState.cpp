...
bool RunState::setDifficulty(int a)
{
    difficulty = a;
      ...
      if(difficulty == 1) {
        ...
        if(!level.load("levels/level2.txt", &factory))
        {
            return false;
        }
    } else if(difficulty == 3) {
        if(!level.load("levels/level3.txt", &factory))
        {
            return false;
        }
    }
...
void RunState::increaseScore(int scoreIncrease)
{
    score += scoreIncrease;
}
